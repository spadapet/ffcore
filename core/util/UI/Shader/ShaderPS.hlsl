...
Texture2D<uint> paletteImage: register(t1);
Texture2D palette: register(t2);
SamplerState patternSampler: register(s0);

Texture2D ramps: register(t3);
SamplerState rampsSampler: register(s3);

Texture2D image: register(t4);
SamplerState imageSampler: register(s4);

Texture2D glyphs: register(t5);
SamplerState glyphsSampler: register(s5);
...
Out main(in In i)
{
    ...
    /////////////////////////////////////////////////////
        ...
        o.color = SampleImage(i.uv1) * (opacity_ * paint.a);

    #elif EFFECT_IMAGE_SHADOW_V
        half alpha = 0;
        half2 dir = half2(0, blurSize * imageSize.w);
        ...
        for (int x = 0; x < GAUSSIAN_NUM_SAMPLES; x++)
        {
            ...
            alpha += weights[x] * 
            (
                SampleImage(clamp(i.uv1 + uvOffset, i.uv2.xy, i.uv2.zw)).a + 
                SampleImage(clamp(i.uv1 - uvOffset, i.uv2.xy, i.uv2.zw)).a
            );
        }
        ...
        half alpha = 0;
        half2 dir = half2(blurSize * imageSize.z, 0);
        half2 offset = half2(shadowOffset.x * imageSize.z, shadowOffset.y * imageSize.w);
        ...
        half4 img = SampleImage(clamp(i.uv1, i.uv2.xy, i.uv2.zw));
        ...
        for (int x = 0; x < GAUSSIAN_NUM_SAMPLES; x++)
        {
            ...
            color += weights[x] * 
            (
                SampleImage(clamp(i.uv1 + uvOffset, i.uv2.xy, i.uv2.zw)) + 
                SampleImage(clamp(i.uv1 - uvOffset, i.uv2.xy, i.uv2.zw))
            );
        }
    ...
    #elif EFFECT_SDF_LCD
        half2 grad = ddx(i.st1);
        half2 offset = grad * imageSize.zw * (1.0 / 3.0);
