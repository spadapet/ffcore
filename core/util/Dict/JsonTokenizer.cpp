...
ff::ValuePtr ff::JsonToken::GetValue() const
{
	switch (_type)
	{
	case JsonTokenType::True:
		return ff::Value::New<ff::BoolValue>(true);

	case JsonTokenType::False:
		return ff::Value::New<ff::BoolValue>(false);

	case JsonTokenType::Null:
		return ff::Value::New<ff::NullValue>();
	...
	case JsonTokenType::Number:
	{
		...
		if (end == _start + _length)
		{
			...
			{
				return ff::Value::New<ff::IntValue>((int)val);
			}
			else
			{
				return ff::Value::New<ff::DoubleValue>(val);
			}
		}
	}
	break;
	...
	case JsonTokenType::String:
	{
		...
		if (cur)
		{
			return ff::Value::New<ff::StringValue>(std::move(val));
		}
	}
	break;
	}
