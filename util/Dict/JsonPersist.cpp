...
static bool JsonWriteValue(const ff::Value* value, size_t spaces, ff::StringOut output);
...
static ff::ValuePtr ParseValue(ff::JsonTokenizer& tokenizer, ff::JsonToken* firstToken, const wchar_t** errorPos)
{
	...
	ff::ValuePtr value = token.GetValue();
	...
	if (!value)
	{
		if (token._type == ff::JsonTokenType::OpenCurly)
		{
			...
			if (!*errorPos)
			{
				value = ff::Value::New<ff::DictValue>(std::move(valueDict));
			}
		}
		else if (token._type == ff::JsonTokenType::OpenBracket)
		{
			...
			if (!*errorPos)
			{
				value = ff::Value::New<ff::ValueVectorValue>(std::move(valueVector));
			}
		}
	}
...
static ff::Dict ParseObject(ff::JsonTokenizer& tokenizer, const wchar_t** errorPos)
{
	...
	for (ff::JsonToken token = tokenizer.NextToken(); token._type != ff::JsonTokenType::CloseCurly; )
	{
		...
		ff::ValuePtr key = token.GetValue();
		if (!key)
		{
			*errorPos = token._start;
			break;
		...
		dict.SetValue(key->GetValue<ff::StringValue>(), value);
...
static bool JsonWriteValue(const ff::Value* value, size_t spaces, ff::StringOut output)
{
	...
	{
		output.append(1, ' ');
		output += JsonEncode(value->GetValue<ff::StringValue>());
	}
	else if (value->IsType<ff::BoolValue>() ||
		value->IsType<ff::NullValue>() ||
		value->IsType<ff::DoubleValue>() ||
		value->IsType<ff::IntValue>())
	{
		ff::ValuePtr strValue = value->Convert<ff::StringValue>();
		output.append(1, ' ');
		output += strValue->GetValue<ff::StringValue>();
	}
	else if (value->IsType<ff::DictValue>() || value->IsType<ff::SavedDictValue>())
	{
		ff::ValuePtr dictValue = value->Convert<ff::DictValue>();

		if (dictValue->GetValue<ff::DictValue>().Size())
		{
			output.append(L"\r\n", 2);
			output.append(spaces, ' ');
		}
		...
		JsonWriteObject(dictValue->GetValue<ff::DictValue>(), spaces, output);
	}
	else if (value->IsType<ff::ValueVectorValue>())
	{
		if (value->GetValue<ff::ValueVectorValue>().Size())
		{
			output.append(L"\r\n", 2);
			output.append(spaces, ' ');
		}
		...
		JsonWriteArray(value->GetValue<ff::ValueVectorValue>(), spaces, output);
	}
	else
	{
		output += L" null";
