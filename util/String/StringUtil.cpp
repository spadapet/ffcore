...
static ff::String StringFromMB(UINT codePage, const char* text, size_t len)
{
	ff::String tstr;
	...
	int bytes = (len == ff::INVALID_SIZE) ? (int)strlen(text) : (int)len;
	tstr.resize(MultiByteToWideChar(codePage, 0, text, bytes, nullptr, 0));
	MultiByteToWideChar(codePage, 0, text, bytes, &tstr[0], (int)tstr.size());
...
{
	return ::StringFromMB(CP_ACP, text, len);
}

ff::String ff::StringFromUTF8(const char* text, size_t len)
{
	return ::StringFromMB(CP_UTF8, text, len);
}
...
static ff::Vector<char> StringToMB(UINT codePage, ff::StringRef text)
{
	int len = (int)text.size() + 1;
	ff::Vector<char> mbText;
	mbText.Resize(::WideCharToMultiByte(CP_ACP, 0, text.c_str(), len, nullptr, 0, nullptr, nullptr));
	::WideCharToMultiByte(CP_ACP, 0, text.c_str(), len, mbText.Data(), (int)mbText.Size(), nullptr, nullptr);
	return mbText;
}

ff::Vector<char> ff::StringToACP(StringRef text)
{
	return ::StringToMB(CP_ACP, text);
}

ff::Vector<char> ff::StringToUTF8(StringRef text)
{
	return ::StringToMB(CP_UTF8, text);
}

ff::Vector<uint32_t> ff::StringToUCS4(StringRef text)
{
	Vector<uint32_t> outText;
...
size_t ff::StringToUCS4(StringRef text, uint32_t* data, size_t dataSize)
{
	for (const wchar_t* src = text.c_str(), *srcEnd = src + std::min(dataSize, text.size() + 1); src != srcEnd; src++, data++)
	{
		*data = *src;
	}
