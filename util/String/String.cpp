...
wchar_t* ff::StringAllocator::Malloc(size_t count)
{
	return ff::ProcessGlobals::Get()->GetStringManager().New(count);
}

void ff::StringAllocator::Free(wchar_t* str)
{
	return ff::ProcessGlobals::Get()->GetStringManager().Delete(str);
}

ff::StringBuffer::StringBuffer()
	: _refs(0)
{
}

ff::StringBuffer::StringBuffer(const StringBuffer& rhs)
	: ff::Vector<wchar_t, 16, ff::StringAllocator>(rhs)
	, _refs(0)
{
}

void ff::StringBuffer::AddRef()
{
	if (_refs != -1)
	{
		_refs.fetch_add(1);
...
void ff::StringBuffer::Release()
{
	if (_refs != -1 && _refs.fetch_sub(1) == 1)
	{
		ff::ProcessGlobals::Get()->GetStringManager().DeleteVector(this);
	}
}

void ff::StringBuffer::DisableRefs()
{
	_refs = -1;
}

bool ff::StringBuffer::IsShared() const
{
	return _refs != 1;
}

class EmptyString : public ff::StringBuffer
{
public:
	EmptyString()
	{
...
{
	return ff::StringFromACP(str);
}

// static
ff::String ff::String::from_utf8(const char* str)
{
	return ff::StringFromUTF8(str);
}
...
ff::StringBuffer& ff::String::Str()
{
	if (_str->IsShared())
	{
		StringBuffer* newStr = ff::ProcessGlobals::Get()->GetStringManager().NewVector(*_str);
		_str->Release();
		_str = newStr;
		_str->AddRef();
	}
...
const ff::StringBuffer& ff::String::StrConst() const
{
	return *_str;
}
