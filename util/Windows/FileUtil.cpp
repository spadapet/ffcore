...
bool ff::CreateDirectory(StringRef path)
{
	...
	// Create the chain of parent directories
	{
		...
		{
			szRealPath = CanonicalizePath(*i, true);
...
#if !METRO_APP
static ff::String GetKnownDirectory(REFGUID id, bool bCreate)
{
	ff::String szReturnPath;
...
bool ff::PathsEqual(StringRef path1, StringRef path2)
{
...
bool ff::PathInPath(StringRef parent, StringRef child)
{
...
bool ff::WriteFile(HANDLE hFile, const void* pMem, size_t nBytes)
{
...
bool ff::ReadWholeFileMemMapped(StringRef path, IData** ppData)
{
	assertRetVal(ppData, false);
	*ppData = nullptr;

	ComPtr<IDataFile> file;
	assertRetVal(ff::CreateDataFile(path, false, &file), false);

	return ff::CreateDataInMemMappedFile(file, ppData);
}
...
bool ff::ReadWholeFile(StringRef path, StringOut szOut)
{
	...
	if (memFile.GetSize() >= 3 && pMem[0] == 0xEF && pMem[1] == 0xBB && pMem[2] == 0xBF)
	{
		// UTF-8
		szOut = StringFromUTF8((const char*)(pMem + 3), memFile.GetSize() - 3);
	}
	else if (memFile.GetSize() >= 2 && pMem[0] == 0xFF && pMem[1] == 0xFE)
	{
		// UTF-16 little endian
		szOut = String((const wchar_t*)(pMem + 2), (memFile.GetSize() - 2) / 2);
	}
...
ff::String ff::CleanFileName(StringRef name)
{
	...
	for (size_t i = 0; i < szNewName.length(); )
	{
